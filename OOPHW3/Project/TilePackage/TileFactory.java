...
import Project.TilePackage.PlayerPackage.*;
import Project.TilePackage.EnemyPackage.*;
...
public class TileFactory {
    ...
    public Enemy produceEnemy(char tile, Position position, MessageCallback messageCallback, EnemyDeathCallback enemyDeathCallBack, ChangedPositionCallback changedPositionCallback, GetTileCallback getTileCallback) {
        Enemy e = enemiesMap.get(tile).get();
        e.initialize(position,messageCallback,enemyDeathCallBack, changedPositionCallback , getTileCallback);
        return e;
    }

    public Player producePlayer(int idx, Position position, MessageCallback messageCallback, ChangedPositionCallback changedPositionCallback){
		Player p = playersList.get(idx-1).get();
        p.initialize(position,messageCallback, changedPositionCallback);
        return p;
    }
